...
async function testApiDb() {
  try {
    console.log('🔧 Testando configuração do banco de dados na API...');
    ...
    // Testar query com where case insensitive
    console.log(`\n7️⃣ Testando query case insensitive:`);
    ...
    if (userCaseInsensitive) {
      console.log(`   ✅ Query case insensitive funcionando: ${userCaseInsensitive.nome}`);
    } else {
      console.log(`   ❌ Query case insensitive falhou`);
    }
    
    console.log(`\n🎉 Configuração do banco na API funcionando perfeitamente!`);

  } catch (error) {
    console.error('❌ Erro na configuração do banco na API:', error);
  } finally {
